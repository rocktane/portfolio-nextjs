  ...
  return (
    <AnimatePresence>
      {active && (
        <motion.div
          key="backdrop"
          className="fixed inset-0 z-[200] flex flex-col items-center justify-center gap-5 bg-black/85 px-4 py-6 backdrop-blur-md"
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
          exit={{ opacity: 0 }}
      ...
      >
        <motion.div
          ...
          style={{
            ...
            background: `linear-gradient(150deg, ${project.accent} 0%, ${project.accent}77 38%, #05050a 100%)`,
            boxShadow: `0 40px 90px -30px ${project.accent}88, 0 2px 0 rgba(255,255,255,0.14) inset`,
          }}
        >
...
function CardFace({ project, muted }: { project: Project; muted: boolean }) {
  ...
  return (
    <div
      className="flex h-full flex-col gap-[1.5cqw] rounded-[2cqw] bg-[#08080c] p-[2.2cqw]"
      aria-hidden={muted}
    ...
    >
      ...
      <div className="flex items-center justify-between gap-[2cqw]">
        {live ? (
          muted ? (
            <span
              className="mono rounded-[0.8cqw] px-[3cqw] py-[1.8cqw] text-[2.1cqw] leading-none text-ink"
